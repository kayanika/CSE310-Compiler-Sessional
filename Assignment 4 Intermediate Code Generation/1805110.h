...
class symbolTable{
    ...
    symbolTable(int sz){
     ...
     string getCurrentScopeTableID(){
        return currentScopeTable->getUniqueId();
     }
    void enterScope(){
   ...
   if(current->getUniqueId()=="1")cout<<"Destroying the first scope"<<endl;
    ...
    symbolInfo* searchSymbol(string item){
        symbolInfo* symbol=currentScopeTable->lookUp(item);
        return symbol;
    }
